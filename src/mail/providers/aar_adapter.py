...
AAR_PREFIX = "aar:"
AAR_BASE_URL_KEY = "aar_base_url"


def _get_aar_base_url() -> str:
    """Lay AAR base URL tu config."""
    cfg = _get_mail_cfg()
    base_url = getattr(cfg, "aar_base_url", None)
    if not base_url:
        # Default: AAR standalone service
    ...
    async with httpx.AsyncClient(timeout=timeout + 10.0) as client:
        try:
            resp = await client.post(
                ...
                json={
                    "session_id": box.token,  # token = session_id
                    "keyword": keyword,
                    "timeout": timeout,
